...
class Game {
  ...
  startLevel(index, seed, keepBoons, withIntro) {
    ...
    for (const p of this.props) if (p.kind === 'rock' || p.kind === 'barrel') this.world.block[Math.floor(p.y / TILE) * this.world.W + Math.floor(p.x / TILE)] = 1;
  ...
  menuPick(i) {
    ...
    if (m.panel === 'levels') {
      ...
      this.audio.sfxCard(); this.startAtLevel(i - 1, m.tripPick);
      return;
    }
    m.index = i;
    const id = MENU[i] || 'new';
  ...
  startAtLevel(li, trip) {
    ...
    this.startLevel(li, this.levelSeed(li), true, li === 0 && this.tripAt !== li);
  }
  ...
  levelCleared() {
    ...
    this.cardQueue = [
      ...
      { code: this.runCode(null), lines: [`SCORE ${score}`, '', `${this.kills} sacrificed in ${this.timer.toFixed(1)}s`,
  ...
  collideEntities(dt) {
    ...
    for (const p of this.props) {
      ...
      for (const e of all) {
        ...
        if (e.state === 'flung' && e !== g && p.kind === 'barrel' && -vn > ph.knockHitSpeed) p.crackBarrel(this);
