...
Blocking: fail 8 (B1 B1a B4 F2 F4 F5 F6 F7), not assessed 5 (B3 B5 B9 B13 G4). Conditional: B12, F8.
```
...
| B4 | No dev tools live | the corner word, the hashes, `KeyN`; grep the shipped files | **fail** | `dev tools` is drawn in the bottom-right on every screen (`render.js:229`, `3048`; `3044` at HEAD), one click from GOD, SPAWN, skip level; `#rules` `#seed=` `#trip` `#dark` (`game.js:117-131`); none of it reaches the run code (probe D) | Claude | yes |
...
| B9 | Zip layout | `index.html` at the zip root, relative paths, only `index.html` + `js/`; list the zip back | **?** | no zip and no itch build step yet. Static pass: every `src` is `js/…`, no absolute path (the probe's one hit is the word `file://` in a comment, `painted-assets.js:4`) | Claude | yes |
...
| G4 | Every floor, THE TRIP and THE DARK played through LEVELS with the console open | the plan's step (it says "all 7 levels"; there are 8 and THE DARK) | **?** | needs a browser | Claude | yes |
...
| F2 | The run code reproduces the floor and says how it was played | codes for lit, dark, easy, god in node; replay a dark code | **fail** | dark = lit, the replay builds the lit floor; easy + god = normal (probe D); the picture card's code is not copied. Uncommitted `game.js` | Claude | yes |
...
| F8 | Anything a tester might read is true | `README.md` against the build | **fail** | "Six levels" (`README.md:4`, and 8 at :77), a pen of three headbutts (7), milk bowls, the Butcher's hits, the seed "top right" (`content-audit.md`, `design-drift.md`) | Claude | only if README is linked |
| F9 | The game's name is one name | `<title>` and the canvas label against README and MARKET | **fail** | `DOOMED GOAT — prototype` (`index.html:5`), `Doomed Goat` (canvas label, `artifact.html` title); GOAT OUT everywhere else | Claude | no |
