...
props are skipped in both prop passes. A held man still draws at the hold point.
...
`nextCard` then sets `game.darkAt` to the next floor, so the dark flight plays THE THRESHING FLOOR dark
(saved with the run; a death keeps it dark). Drawn cold (`drawStairs(..., cold)`, `Renderer.forkRow`),
THE LAMPS ARE LIT / ARE OUT on the floor in front of each (`drawHints`). `GEN_RULES.fork`; `balance.js`
holds the dark floor under its lit twin and above `fork.band` of it. Anything that reads `exitTile`
still means the lit flight (compass, escorts, the bell's thread).
...
place of it, via `game.darkAt` (set by `forgetLessons` / `resumeRun`, and by THE FORK's dark flight).
`dark.runAt` forces one floor of every run dark (-1 since 24 Sep 2026: it was THE CAVE; the dark is
a choice at THE FORK now). LEVELS can play any floor dark (its own toggle row, `menu.darkPick`, next
to THE TRIP's; one on turns the other off), and so can `#dark`. Unlike the trip it stays that floor to whatever asks which floor it is:
...
have, touches no save; its first `LEVEL_TOGGLES` rows are switches — `menu.tripPick` plays
`tripLevel(li)`, `menu.darkPick` plays `darkLevel(li)`. `SETTINGS` / `game.settings` /
