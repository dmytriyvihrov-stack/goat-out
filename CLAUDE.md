...
games are the real genre neighbors. Read it before starting a fresh art pass.
...
| `js/combat-assets.js` | `COMBAT_ASSETS`: the combat-effects sheet, generated by `tools/pack-combat-art.cjs`. Never edit by hand. |
| `js/combat-fx.js` | `CombatFX`: cosmetic fragments and bursts (deaths, body pieces) off that sheet; never enters collision, damage, noise or AI. |
...
BOMB CHARGE, SPLASH (`TUNING.goat.hornLooks`; `hornMods` shares the lean). The worn talisman hangs from
...
(`drawStatusTab`), TALISMANS, FIXTURES, JUICE, MUSIC.
...
0 `WASD — TO MOVE` in the pen above `cagePrompt`; 1 grab/throw in the ambush; 2 the headbutt, one line,
...
time. `daze` × `cfg.dazeMul`, cancels a dart. `game.houndSeen()`.
...
`chargeSkid`; the wall stun (`chargeStopped`) is earned by standing at a wall. Charging he smashes doors,
...
**Boons.** `applyBoons()` rebuilds `game.mods` from `game.boons`; use sites read `game.mods`, never mutate
`TUNING`. New boon: `BOONS`, `BOON_BASE`, the use site; `skill` hangs it on a button; `needs` gates the
...
lands. Chips show the key; `drawSkillNote` (`renderer.skillHover`) the name and a mod-aware `note`. HUD
...
`goat.state === 'stunned'` is real (`game.stunGoat` only).

**Roll.** Born in full (`BOON_BASE.roll`); DEAD WEIGHT = `mods.rollStun`. `Goat.rollDirection` scores 24
angles. Distance is `roll.speed` × `roll.duration`; tune speed, not mercy frames.
...
**Barrels.** `kind === 'barrel'` (`TUNING.prop.barrel`, `levelDef.barrels` per room, not the cave):
blocking, stops bullets, not an item, in `world.block` until broken (`openBarrel`). `crackBarrel` on two
blows or a flung body past `knockHitSpeed`; fire at or beside it is `burstBarrel`, a burning oil pool.
`barrelFits`: wall on exactly one side, open floor round the rest, clear of `room.enter`. `GEN_RULES.barrels`.
...
(`ARTIFACT_HOW`, `MILK_OFFER.how`, `wareNote`). **A tier `desc` is literal numbers** (I whole, II/III the
diff), shown only there and on the chip hover — **change a param, change its line.**
