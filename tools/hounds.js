...
window.HOUNDS = {
  ...
  crowd(levels, seeds, { men = 2, mode = 'post', secs = 15, min = 6 } = {}) {
    ...
    let tries = 0, never = 0, died = 0, time = 0, lean = 0, stuck = 0; const where = []; HOUNDS.fails = []; HOUNDS.scenes = [];
    try {
      for (const li of levels) for (const seed of seeds) {
        Math.random = real;
        HOUNDS.level(li, seed * 104729);
        ...
        for (const r of rooms) {
          ...
          const e = new Enemy(a.x, a.y, 'dog'); e.aware = true; e.woke = true; e.state = 'chase'; game.enemies.push(e);
          let ok = false, t = 0, leant = 0; const rec = [], goatRec = [];
          for (let st = 0; st < secs * 60 && !ok; st++) {
            ...
            if (posts.some((q) => !q.e.dead && len(q.e.x - e.x, q.e.y - e.y) < q.e.r + e.r + 3)) { lean += 1 / 60; leant += 1 / 60; }
          ...
          HOUNDS.last = { li, seed, ri: r.index, rec: [rec], goatRec, lean: leant, men: posts.map((q) => ({ x: q.e.x, y: q.e.y, r: q.e.r })) };
          HOUNDS.scenes.push(HOUNDS.last);
