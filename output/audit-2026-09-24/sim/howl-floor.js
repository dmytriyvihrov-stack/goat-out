...
g.frame = function () {};
const FLOOR = +(process.argv[4] || 0);
if (FLOOR) { const ab = g.applyBoons.bind(g); g.applyBoons = function () { ab(); this.mods.screamCooldown = Math.max(this.mods.screamCooldown, FLOOR); }; }
...
console.log(`WHAT-IF floor ${FLOOR}s; scream stun ${TUNING.goat.scream.stun}s, hound dazeMul ${TUNING.dog.dazeMul}, level ROAD, ${SEEDS} seeds, 30 s, goat stands and screams when ready`);
console.table(rows);
