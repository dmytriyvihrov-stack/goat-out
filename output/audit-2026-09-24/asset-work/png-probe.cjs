...
for (const f of process.argv.slice(2)) {
  if (f.endsWith('.png')) { out.push(variants(path.basename(f), fs.readFileSync(f))); continue; }
  const text = fs.readFileSync(f, 'utf8'), re = /"?([A-Za-z_$][\w$]*)"?\s*:\s*\{[^{}]*?"?src"?\s*:\s*"data:image\/png;base64,([A-Za-z0-9+/=]+)"|"src"\s*:\s*"data:image\/png;base64,([A-Za-z0-9+/=]+)"/g;
  let m;
  while ((m = re.exec(text))) out.push(variants(path.basename(f) + ':' + (m[1] || 'src'), Buffer.from(m[2] || m[3], 'base64')));
}
